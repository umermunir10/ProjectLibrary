...
public extension String {
    ...
    ///
    ///
    ///        "F@raz123".containsSpecialCharacters -> true
    ///
...
#if canImport(Foundation)
    /// SwifterSwift: Check if string is a Phone Number.
    ///
    ///        "03344211800".isValidPhoneNumber -> true
    ///        "google.com".isValidPhoneNumber -> false
    ///
...
#if canImport(Foundation)
    /// Muzamil Mughal: Checks if the string contains upper and lower case alphabets, digits, and special characters
    ///
    ///        "Swift is amazing".vaildPasswordContaining -> true
    ///
    /// - Returns: true or false if string complies with parameters of the function
    func validPasswordContaining(_ upperCaseCharacters: Bool = true, _ lowerCaseCharacters: Bool = true, _ digits: Bool = true, _ specialCharacters: Bool = true) -> Bool {
        
        if upperCaseCharacters {
            
            let upperCaseCharactersRegex = ".*[A-Z].*"
            if !(range(of: upperCaseCharactersRegex, options: .regularExpression, range: nil, locale: nil) != nil) {
                return false
            }
        }
        ...
        if lowerCaseCharacters {
            ...
            let lowerCaseCharactersRegex = ".*[a-z].*"
            if !(range(of: lowerCaseCharactersRegex, options: .regularExpression, range: nil, locale: nil) != nil) {
                return false
            }
        }
        ...
        if digits {
            ...
            let digitsRegex = ".*[0-9].*"
            if !(range(of: digitsRegex, options: .regularExpression, range: nil, locale: nil) != nil) {
                return false
            }
        }
